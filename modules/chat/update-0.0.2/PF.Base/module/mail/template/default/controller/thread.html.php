...
<div id="js_main_mail_thread_holder">
	...
	*}
	<div id="mail_messages{$userId}" class="mail_messages">
    {literal}
      <script>
        var flPage = 2,
          mainChatFromId = $("#js_mail_thread_view_more").attr('rel');
        function mailViewMore(idThread){
          $.ajaxCall('mail.viewMoreThreadMail', 'page='+flPage+'&thread_id='+idThread, 'GET');
          flPage++;
        }
      </script>
    {/literal}
		{if count($aMessages) >= 10}
			<a id="js_mail_thread_view_more" onclick="mailViewMore({$aThread.thread_id});" class="mail_view_more no_ajax_link" rel="{$userId}">{phrase var='mail.view_more'}</a>
		{/if}
		...
		<div id="mail_write_now_mainchat{$userId}" class="mail_write_now">{img theme='layout/typing_blue.gif'}<span></span></div>
	</div>
	{literal}
	<script>
		function mailOnSuccess(e, obj) {
			$(".mail_messages").animate({ scrollTop: $('.mail_messages')[0].scrollHeight}, 0);
			$("#message").val("");
			$("#message").focus();
		};
	...
	<div class="mail_thread_form_holder">
		<div class="mail_thread_form_holder_inner">
			{$sCreateJs}
			<form method="post" action="{url link='mail.thread' id=$aThread.thread_id}" id="js_form_mail" class="ajax_post" data-validation-start="validationStart" data-callback="mailOnSuccess">
				...
				<textarea id="message" name="val[message]" class="on_enter_submit" placeholder="Введите сообщение"></textarea>
				<input id="button_message_send" type="submit" style="display:none;" value=""></input>
				<label for="button_message_send" class="button_message_send"><i class="fa fa-envelope fa-2x"></i></label>
			</form>
		</div>
	</div>
	{* moderation *}
