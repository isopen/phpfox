...
class Mail_Component_Controller_Thread extends Phpfox_Component
{
	...
	public function process()
	{
		...
		if ($aVals && ($iNewId = Mail_Service_Process::instance()->add($aVals)))
		{
			...
			//Message in Socket!!!Maxim Tyulenev

			$threadId = Mail_Service_Mail::instance() -> getThreadedMail($aVals["thread_id"])[0];
			foreach ($threadId['users'] as $aUser){
			...
			Chat_Service_Chat::instance() -> sendSocket($idUser,"message",$aVals["message"],0,0);

			//end message in Socket!!!
			...
			return [
				'append' => [
					'to' => '#mail_threaded_new_message,#box_user_message'.$idUser, // add box_user_message!!!Maxim Tyulenev
					'with' => $content
				]
			];
		...
		$idUser = $this->request()->getInt('user_id');
		...
		$threadId = Mail_Service_Mail::instance() -> getThreadedMail(1);
		$this->template()->setTitle($sUsers)
			...
			->assign(array(
					...
					'aMessages' => $aMessages,
					'aThread' => $aThread,
					'userId' => $idUser, //add param. Maxim Tyulenev.
					'thread' => json_encode($threadId),
