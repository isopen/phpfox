...
$Core.Chat = {
	init:function(){
		...
		$Core.ajax("chat.index",{
			success:function($sOutput){
				...
				if(window.location.pathname.indexOf("admincp")+1){
					$Core.Chat.exitChat();
				}
	...
	selMessages:function(idThread,idUser,i){
		...
		$Core.ajax("chat.messages",{
			...
			success:function($sOutput){
				...
				$("#panel_rows_user"+idUser).removeClass("is_new");
				$("#js_total_new_messages").css("display","none");
				$("#js_total_new_messages").empty();
	...
	scroll:function(userId){
		...
		if($('.mail_messages')[0]){
			$("#mail_messages"+userId).animate({ scrollTop: $('.mail_messages')[0].scrollHeight}, 0);
		}
	...
	updateView:function(userId,message){
		...
		if($("#box_user_message"+userId).length){
			...
			$(".body_chat > a[name='friend_chat"+userId+"'] > .status_friend").addClass("status_message");
	...
	updateOnline:function(e){
		...
		var elem = $(".body_chat > a[name='friend_chat"+e.userId+"'] > .status_friend_hide");
		elem.removeClass("status_friend_hide");
		elem.addClass("status_friend");

		elem = $(".body_chat > a[name='friend_chat"+e.userId+"']"),
		//clone = elem.clone();
		elem.detach();
		$(".body_chat").prepend(elem);

	},
	updateOffline:function(e){

		var elem = $(".body_chat > a[name='friend_chat"+e.userId+"'] > .status_friend");

		if(!elem.hasClass("status_message")){

			elem.removeClass("status_friend");
			elem.addClass("status_friend_hide");

			elem = $(".body_chat > a[name='friend_chat"+e.userId+"']");
			elem.detach();
			$(".body_chat").append(elem);
...
$Core.socket = {
	init:function(){
		...
		var location = window.location,
				socket = new ReconnectingWebSocket("ws://"+location.hostname+":8080");
		socket.debug = true;
		...
		socket.onmessage = function(e){
			...
			switch(e.type){
				...
				break;
				case "online":
					$Core.Chat.updateOnline(e);
				break;
				case "offline":
					$Core.Chat.updateOffline(e);
				break;
				case "write":
					$Core.Chat.writeUser(e.userId,e.message);
				break;
				case "addFriend":
					$Core.Chat.updateFriend();
				break;
				case "removeFriend":
					$Core.Chat.updateFriend();
				break;
